function valid_datas( f ){
	...
	if( f.name.value == '' ){
		jQuery('#form_status').html('<span class="wrong">Geben Sie mindestens ihren Nachnamen ein!</span>');
		notice( f.name );
	}else if( f.email.value == '' ){
		jQuery('#form_status').html('<span class="wrong">Geben Sie eine gültige Email an!</span>');
		notice( f.email );
	//}else if( f.phone.value == '' ){
		//jQuery('#form_status').html('<span class="wrong">Your phone must not be empty and correct format!</span>');
		//notice( f.phone );
	}else if( f.subject.value == '' ){
		jQuery('#form_status').html('<span class="wrong">Geben Sie einen Betreff an!</span>');
		notice( f.subject );
	}else if( f.message.value == '' ){
		jQuery('#form_status').html('<span class="wrong">Geben Sie eine Nachricht an!</span>');
		notice( f.message );
	}else{
		 jQuery.ajax({
			url: 'mail.php',
		...
		jQuery('#form_status').html('<span class="loading">Die Nachricht wird verschickt...</span>');
